...
@ccclass('GetReadyScaler')
export default class GetReadyScaler extends Component {
    ...
    updateScale() {
        ...
        if (this.label) {
            this.label.fontSize = isPortrait ? this.portraitFontSize : this.landscapeFontSize;
        }

        if (this.cursor) {
            const s = isPortrait ? this.portraitCursorScale : this.landscapeCursorScale;
